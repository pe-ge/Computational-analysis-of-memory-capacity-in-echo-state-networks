...
"""
import numpy as np
from numpy import random, zeros, tanh, dot, corrcoef, average, hstack
import scipy.linalg
...
def memory_capacity(W, WI, memory_max=None,
    ...
    errors = np.zeros(memory_max)
    stds = np.zeros(memory_max)
    for h in range(memory_max):
        k = h + 1
        e = u[memory_max - k : memory_max + iterations_coef_measure - k] - o[h, : ]
        errors[h] = np.mean(e)
        stds[h] = np.std(e)
